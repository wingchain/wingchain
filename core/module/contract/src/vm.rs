...
use executor_primitives::{
    self, CallEnv, Context, ContextEnv, EmptyParams, Module, ModuleError, ModuleResult, SEPARATOR,
    Util,
};
...
impl<M: Module> VMContext for DefaultVMContext<M> {
    ...
    fn module_balance_transfer(
        ...
        module_balance::Module::<M::C, M::U>::validate_transfer(&self.executor_util, params.clone())
            .map_err(Self::module_to_vm_error)?;
        let balance_module =
            module_balance::Module::new(self.module_context.clone(), self.executor_util.clone());
...
#[derive(Clone)]
pub struct DummyExecutorContext<U: Util> {
    util: U,
	payload: Rc<RefCell<HashMap<DBKey, DBValue>>>,
	events: Rc<RefCell<Vec<Event>>>,
}

impl<U: Util> DummyExecutorContext<U> {
...
impl<U: Util> Context for DummyExecutorContext<U> {
    fn env(&self) -> Rc<ContextEnv> {
        Rc::new(ContextEnv {
            number: 0,
            timestamp: 0,
