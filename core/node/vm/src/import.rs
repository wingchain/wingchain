...
fn balance_transfer(ctx: &mut Ctx, recipient_address_len: u64, recipient_address_ptr: u64, value: u64) -> VMResult<()> {
    let state = get_state(ctx);
    let memory = &state.memory;

    let sender_address = &state.context.call_env().contract_address;
    ...
    state.context.transfer(sender_address, &recipient_address, value)?;
