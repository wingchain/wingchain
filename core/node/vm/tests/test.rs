...
use node_vm::{VM, VMCallEnv, VMConfig, VMContext, VMContextEnv};
...
fn test_vm_balance_transfer() {
    ...
    let input = serde_json::to_vec(&input).unwrap();
    let result = vm_execute(context.clone(), "balance_transfer", input);
}
...
impl VMContext for TestVMContext {
    ...
    fn transfer(&self, sender: &Address, recipient: &Address, value: Balance) -> VMResult<()> {
        let mut guard = self.balances.borrow_mut();
        let mut sender_balance = guard.get(sender).unwrap_or(&0).clone();
        if sender_balance < value {
            return Err(BusinessError::Transfer.into());
        }
        let mut recipient_balance = guard.get(recipient).unwrap_or(&0).clone();

        sender_balance -= value;
        recipient_balance += value;

        guard.insert(sender.clone(), sender_balance);
        guard.insert(recipient.clone(), recipient_balance);
