...
use crate::base::{storage_mint, ExecutorContext, TestExecutorContext, TestVMContext};

mod base;

#[test]
fn test_vm_hw_validate_contract() {
	// input deserialize error
	let input = r#"{"value1":"abc"}"#;
	let error = vm_validate(None, Mode::Init, "init", input, 0).unwrap_err();

	let expected_error: VMError = ContractError::Deserialize.into();

	assert_eq!(format!("{:?}", error), format!("{:?}", expected_error));

	// not payable error
	let input = r#"{"value":"abc"}"#;
	let error = vm_validate(None, Mode::Init, "init", input, 10).unwrap_err();
	...
	// pre compile error
	let input = r#"{"value":"abc"}"#;
	let error = vm_validate(Some(vec![1; 1024]), Mode::Init, "init", input, 0).unwrap_err();
...
fn test_vm_hw_init() {
	...
	let input = r#"{"value":"abc"}"#;
	let result = vm_execute(&context, Mode::Init, "init", input, 0).unwrap();

	assert_eq!(result, r#"null"#.to_string());

	let input = r#""#;
	let result = vm_execute(&context, Mode::Call, "get_value", input, 0).unwrap();
...
fn test_vm_hw_hello() {
	...
	let result = vm_execute(&context, Mode::Call, "hello", input, 0).unwrap();
...
fn test_vm_hw_pay_value() {
	...
	let input = r#""#;
	let result = vm_execute(&context, Mode::Call, "pay_value", input, 100).unwrap();
...
fn test_vm_hw_error() {
	...
	let input = r#""#;
	let error = vm_execute(&context, Mode::Call, "error", input, 0).unwrap_err();
...
fn test_vm_hw_get_env() {
	...
	let input = r#""#;
	let result = vm_execute(&context, Mode::Call, "get_env", input, 0).unwrap();
...
fn test_vm_hw_get_call_env() {
	...
	let input = r#""#;
	let result = vm_execute(&context, Mode::Call, "get_call_env", input, 0).unwrap();
...
fn test_vm_hw_get_contract_env() {
	...
	let input = r#""#;
	let result = vm_execute(&context, Mode::Call, "get_contract_env", input, 0).unwrap();

	assert_eq!(result, r#"{"contract_address":"ca5d3fa0a6887285ef6aa85cb12960a2b6706e00","sender_address":"b4decd5a5f8f2ba708f8ced72eec89f44f3be96a"}"#.to_string());
}
...
fn test_vm_hw_value() {
	...
	let input = r#"{"value":"abc"}"#;
	let result = vm_execute(&context, Mode::Call, "set_value", input, 0).unwrap();
	assert_eq!(result, r#"null"#.to_string());
...
fn test_vm_hw_map() {
	...
	let result = vm_execute(&context, Mode::Call, "get_map", input, 0).unwrap();
	assert_eq!(result, r#"{"value":null}"#.to_string());

	let input = r#"{"key":[1,2,3],"value":"abc"}"#;
	let result = vm_execute(&context, Mode::Call, "set_map", input, 0).unwrap();
	assert_eq!(result, r#"null"#.to_string());
...
fn test_vm_hw_event() {
	...
	let input = r#""#;
	let _result = vm_execute(&context, Mode::Call, "event", input, 0).unwrap();
...
fn test_vm_hw_hash() {
	...
	let input = r#"{"data":[1]}"#;
	let result = vm_execute(&context, Mode::Call, "hash", input, 0).unwrap();
...
fn test_vm_hw_address() {
	...
	let input = r#"{"data":[1]}"#;
	let result = vm_execute(&context, Mode::Call, "address", input, 0).unwrap();
...
fn test_vm_hw_validate_address() {
	...
	let error = vm_execute(&context, Mode::Call, "validate_address", input, 0).unwrap_err();
	...
	let input = r#"{"address":"aa"}"#;
	let result = vm_execute(&context, Mode::Call, "validate_address_ea", input, 0).unwrap();
...
fn test_vm_hw_balance() {
	...
	let result = vm_execute(&context, Mode::Call, "get_balance", input, 0).unwrap();
...
fn test_vm_hw_balance_transfer_success() {
	...
	let result = vm_execute(&context, Mode::Call, "balance_transfer", &input, 100);
...
fn test_vm_hw_balance_transfer_partial_failed() {
	...
	let result = vm_execute(&context, Mode::Call, "balance_transfer_ea", &input, 100);
...
fn vm_execute(
	...
	let code = code();
	base::vm_execute(code, context, mode, method, input, pay_value)
}
...
fn vm_validate(
	...
	base::vm_validate(&*code, mode, method, input, pay_value)
}
